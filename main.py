...
from actions.volume import set_volume, set_mute
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "set_system_volume":
                ...
                if action == "mute":
                    result = await loop.run_in_executor(None, lambda: set_mute(True))
                elif action == "unmute":
                    result = await loop.run_in_executor(None, lambda: set_mute(False))
                else:
                    if "percent" not in args:
                        raise ValueError("Ses yüzdesi belirtilmedi.")
                    result = await loop.run_in_executor(None, lambda: set_volume(int(args.get("percent"))))
